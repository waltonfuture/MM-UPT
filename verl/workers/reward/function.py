...
class FunctionRewardManager:
    ...
    def __call__(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        ...
        for i in range(len(data)):
            ...
            extracted_response = extract_boxed_content(response_str)
            extracted_answers.append(extracted_response)
            v.append(valid_response_length)
        
        majority_items = []

        original_batch_size = len(data) // n
        for i in range(original_batch_size):
            # Get responses for this group
            group_answers = extracted_answers[i*n:(i+1)*n]
            # Find majority item in this group
            group_counter = Counter(group_answers)
            group_majority = group_counter.most_common(1)[0][0]
            # Store majority item for each response in this group
            majority_items.extend([group_majority] * n)


        for i in range(len(data)):
            valid_response_length = v[i]
            score = 1.0 if grade_answer(extracted_answers[i], majority_items[i]) else 0.0
